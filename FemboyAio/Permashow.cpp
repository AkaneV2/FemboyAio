...
void Permashow_OnDraw()
{
	...
	if (keyboard_state->is_pressed(keyboard_game::mouse1) && orbwalker->none_mode())
	{
...
{
	Initialized = true;
	permashow = main_tab->add_tab("fatality.permashow", "Permashow");
	{
		permashow_enabled = permashow->add_checkbox("fatality.permashow.enabled", "Enabled", true);
		//permashow->set_assigned_active(permashow_enabled);

		permashow_pos_x = permashow->add_slider("fatality.permashow.posx", "Position X", int(renderer->screen_width() * 0.65f), 0, renderer->screen_width());
		permashow_pos_y = permashow->add_slider("fatality.permashow.posy", "Position Y", int(renderer->screen_height() * 0.733f), 0, renderer->screen_height());

		permashow_fontsize = permashow->add_slider("fatality.permashow.font", "Font size", 16, 9, 25);
		permashow_fontsize->add_property_change_callback(Permashow_FontChange);
		auto colors = permashow->add_tab("fatality.permashow.colors", "Colors");
		{
			background_color = colors->add_colorpick("fatality.permashow.background_color", "Background", { 14.f / 255.f, 22.f / 255.f, 23.f / 255.f, 200.f / 255 });
			text_color = colors->add_colorpick("fatality.permashow.text_color", "Color", { 255.f / 255.f, 255.f / 255.f, 255.f / 255.f, 255.f / 255.f });
			border_color = colors->add_colorpick("fatality.permashow.border_color", "Border", { 124.f / 255.f, 106.f / 255.f, 73.f / 255.f, 255.f / 255.f });
			separator_color = colors->add_colorpick("fatality.permashow.separator_color", "Separator", { 0.f / 255.f, 55.f / 255.f, 49.f / 255.f, 255.f / 255.f });
			title_color = colors->add_colorpick("fatality.permashow.title_color", "Title", { 143.f / 255.f, 122.f / 255.f, 72.f / 255.f, 255.f / 255.f });
			title_background_color = colors->add_colorpick("fatality.permashow.title_background_color", "Title Background", { 19.f / 255.f, 30.f / 255.f, 32.f / 255.f, 230.f / 255.f });
		}
	}
